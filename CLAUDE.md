...
**gemma4:e4b** via Ollama (`WES_LLM=local`; streaming + tools + vision, one resident
model for both chat and scene description; Claude Haiku is the fallback/optional
backend) → piper TTS streamed sentence-by-sentence to a Bluetooth JBL.
...
- `pc/run_server.ps1` — PC launcher (voice/env); `tests/` — the test suite.
...
- Anything latency-affecting → `& $py Z:\wes\tests\perf_check.py` (flags regressions vs
  baseline; records to `tests/perf_history.csv`)
