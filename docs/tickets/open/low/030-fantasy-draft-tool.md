...
### 2026-07-30 — DEPRIORITIZED med -> low: the owner will draft manually
Owner call: "let's deprioritize draft stuff for now. I'll likely end up drafting
manually myself." The autonomous draft agent is no longer on the critical path,
so the one time-boxed item on the board (recon against a live draft room before
the LSE league drafts) is dropped rather than raced.

What survives and still matters, because #029 uses it in-season:
- `wes_draft.best_available` + the NBA z-score ranker stay — they are the
  waiver/pickup recommender too, not draft-only.
- `wes_nfl`'s points valuer and the ESPN pool stay (they were built under #029).
What is shelved: the live Yahoo draft-room automation (board scraping, on-the-
clock detection, pick submission) and the DOM recon it needs.
